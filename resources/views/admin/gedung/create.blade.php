	...
	<form method="POST" action="{{ route('gedung.store') }}">
		...
		<div class="space-y-4">
			<div>
				...
				<input type="text" name="nama" value="{{ old('nama') }}" required minlength="3" maxlength="255"
					class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
				@error('nama')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
			</div>
			<div>
				<label class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
				<input type="text" name="lokasi" value="{{ old('lokasi') }}" maxlength="255"
			...
			<div class="grid grid-cols-2 gap-4">
				<div>
					...
					<input type="number" name="kapasitas" value="{{ old('kapasitas') }}" min="1" max="10000"
						class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
			...
			<div>
				...
				<textarea name="deskripsi" rows="4" maxlength="1000"
					class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="Deskripsi gedung...">{{ old('deskripsi') }}</textarea>
